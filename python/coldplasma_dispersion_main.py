...
def dispersion(theta, w):
    ...
    k1, k2 = w/c*np.sqrt(n1), w/c*np.sqrt(n2)
    return k1,k2

theta = 0
alpha = 80
w = abs(omega_o)*np.arange(0.01, 2, 0.01) + 0.1
...
plt.figure()
plt.scatter(k1, w/abs(omega_o), c=Eo, cmap = cm, label=r'$k+$')
plt.scatter(k2, w/abs(omega_o), c=Eo, cmap = cm, label=r'$k-$')
